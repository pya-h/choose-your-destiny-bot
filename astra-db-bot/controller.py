...
class GameController:
    ...
    def telltale(self):
        ...
        while not is_game_ended:
            answer, is_game_ended = self.react(choice)
            print(answer)
            choice = input('What do you want to do? ')

    def react(self, human_choice):
        answer = self.llm_chain.predict(human=choice).strip()
        return answer, '**The End**' not in response
